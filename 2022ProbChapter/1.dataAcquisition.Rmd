...
source('C:/HardDriveBackup/R/GitHub/WQMdataQueryTool/conventionalsFunction12142021.R')
...
#write.csv(conventionals, 'processedData/2020conventionals.csv', row.names = F, na = "")
```
...
## This part became useless after conventionals12142021.R update
...
```{r one row per year}
lucy <- read_excel('processedData/prob20192020data_LKSEdits.xlsx', sheet = 'prob20192020data_EVJfinalEdits', skip = 1) %>% # skip first row
...
#write.csv(lucyFin, 'processedData/FinalFieldAnalyteMetalsFinal.csv', row.names = F, na= '')
...
```{r field}
field <- read.csv('processedData/FinalFieldAnalyteMetalsFinal.csv') %>% #FinalFieldAnalyteMetals.csv') %>% 
  rename('Ortho-P' = 'Ortho.P',
         'Hg-C' = 'HgCom', 
         '70331VFine' = 'X70331VFine')
...
finalSmashMetadata <- mutate(finalSmashMetadata,
                            ...
                            Panel = case_when(between(Year, 2001, 2010)~ 'Phase1',
                                              between(Year, 2011, 2020)~ 'Phase2',
                                              TRUE ~ as.character(NA)),
...
designStatusOld <- read_excel('C:/HardDriveBackup/R/GitHub/FreshwaterProbMonIntegratedReports/2020ProbChapter/processedData/biology20012018final.xlsx', sheet = 'biology2020') %>% 
  left_join(dplyr::select(prob2022fin, StationID_Trend, AREA_SQ_MILES),
            by = c('sampleID' = 'StationID_Trend')) %>% 
  mutate(IR2022 = case_when(between(Year, 2015, 2020) ~ 2022, TRUE ~ as.numeric(NA)), 
         
         VAHUSB = NA, VAHUSB_NAME = NA,
