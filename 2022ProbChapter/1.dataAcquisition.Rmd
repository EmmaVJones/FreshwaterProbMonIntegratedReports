...
finalSmashMetadata <- mutate(finalSmashMetadata,
                            IR2008 = case_when(between(Year, 2001, 2007)~ 2008, TRUE ~ as.numeric(1)),
                            IR2010 = case_when(between(Year, 2003, 2008)~ 2010, TRUE ~ as.numeric(1)),
                            IR2012 = case_when(between(Year, 2005, 2010)~ 2012, TRUE ~ as.numeric(1)),
                            IR2014 = case_when(between(Year, 2007, 2012)~ 2014, TRUE ~ as.numeric(1)),
                            IR2016 = case_when(between(Year, 2009, 2014)~ 2016, TRUE ~ as.numeric(1)),
                            IR2018 = case_when(between(Year, 2011, 2016)~ 2018, TRUE ~ as.numeric(1)),
                            IR2020 = case_when(between(Year, 2013, 2018)~ 2020, TRUE ~ as.numeric(1)),
                            IR2022 = case_when(between(Year, 2015, 2020)~ 2022, TRUE ~ as.numeric(1)),
...
What's the breakdown of this new category?
...
prob2022sf <- prob2022 %>% 
  ...
  dplyr::select(DataSource:VAHUSB, Basin4thOrder, BayShed:STXRD) %>% 
  arrange(Year, StationID) %>% 
  st_drop_geometry()

write.csv(prob2022sf, 'processedData/prob2022sf.csv', row.names = F, na  ='')

write.csv(prob2022sf %>% 
  group_by(Basin) %>% 
  count(), 'processedData/BasinSummary.csv', row.names = F, na  ='')


write.csv(prob2022sf %>% 
  group_by(SubBasin) %>% 
  count, 'processedData/SubBasinSummary.csv', row.names = F, na  ='')

write.csv(prob2022sf %>% 
  group_by(VAHUSB) %>% 
  count(), 'processedData/VAHUSBSummary.csv', row.names = F, na  ='')

write.csv(prob2022sf %>% 
  group_by(Basin4thOrder) %>% 
  count(), 'processedData/Basin4OrderSummary.csv', row.names = F, na  ='')
```
...
designStatusNew <- read_excel('originalData/Wadeable_ProbMon_2019-2020_Final.xlsx', 
  ...
  left_join(dplyr::select(finalSmashMetadata, StationID, `strahler order` = Order, 
