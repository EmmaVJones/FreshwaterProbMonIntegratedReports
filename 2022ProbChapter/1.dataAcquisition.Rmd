...
write.csv(filter(SCI_filter, `Collected By` %in% c("BRETT STERN", "MIKEJ SHAVER", "JUSTIND LOYD" )), 'VSCI_VCPMIreview.csv', row.names = F, na = '')

SCI_filterFin <- SCI_filter %>% 
  filter(SCI != 'VSCI')

bugAvg <- SCI_filter %>% 
  group_by(StationID, Year, SCI) %>% 
  summarise_at(vars(`Family Total Taxa`:PctIntol), mean, na.rm = TRUE) %>% 
  dplyr::select(TotTaxa = 

TotTaxa"                 "EPTTax"                 
 [66] "VEphem"                  "VPTHydropsychidae"       "VScrap"                  "VChiro"                  "V2Dom"                  
 [71] "HBI"                     "VHapto"                  "EPTInd"                  "VSCIVCPMI
names(prob2020)
names(bugAvg)
```
