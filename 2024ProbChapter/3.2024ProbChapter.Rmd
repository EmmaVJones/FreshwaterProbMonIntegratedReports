...
basinssmooth <- st_read('originalData/GIS/VAbasins_smoothNoChesPeeDee.shp')# basin shapefile ggplot maps (sf)
basinssmooth_sp <- readOGR('originalData/GIS','VAbasins_smoothNoChesPeeDee')# basin shapefile for micromaps (sp)
VAoutline <- st_read('originalData/GIS/Va_otlne.shp') # for basic site maps
VAbasins <- st_read('originalData/GIS/basinsstatewide.shp') # for basic site maps
VAecoregionsL3 <- st_read('originalData/GIS/VA_level3ecoregion.shp') # for basic site maps

### update to 2020 when available

allProb <- read_csv('processedData/Wadeable_ProbMon_2001-2020.csv') %>% # for basic site maps
  dplyr::select(DataSource:IR2022) %>%
  st_as_sf(coords = c("LongitudeDD", "LatitudeDD"),  # make spatial layer using these columns
             remove = T, # don't remove these lat/lon cols from df
             crs = 4326) # add coordinate reference system, needs to be geographic for now bc entering lat/lng, 
boatableSites <- st_read('processedData/GIS/boatableSites2006_2020.shp')
...
ggplot() +
  ...
  geom_sf(data = filter(allProb, IR2022 == 2022), 
          aes(color = 'IR2022 Probabilistic Sites (Wadeable)'),
          size = 1, show.legend = "point") +
  coord_sf(datum = NA) + # take away grid
  scale_colour_manual(values = c('IR2022 Probabilistic Sites (Wadeable)' = 'black'),
...
##### Figure 4.4-3. Virginia probabilistic monitoring locations from `r IRrange` with Virginia major river basins (n = `r nrow(filter(allProb, IR2022 == 2022))`).
...
ggplot() +
  ...
  geom_sf(data = filter(allProb, IR2022 == 2022), 
          aes(color = 'IR 2022 Probabilistic\n Sites (Wadeable)'),
          size = 1, show.legend = 'point') +
  coord_sf(datum = NA) + # take away grid
  scale_fill_manual(values= col.regions, name = NULL,
                    guide = guide_legend(override.aes = list(linetype = 'solid',
                                                             shape = NA))) +
  scale_colour_manual(values = c('IR 2022 Probabilistic\n Sites (Wadeable)' = 'black'),
...
##### Figure 4.4-4. Virginia probabilistic monitoring locations from `r IRrange` with US Level III Ecoregions (n = `r nrow(filter(allProb, IR2022 == 2022))`).
...
ggplot() +
  ...
  coord_sf(datum = NA) + # take away grid
  scale_colour_manual(values = c('IR 2022 Wadeable Sites' = 'black',
...
##### Table 4.4-1. Dissolved oxygen results (`r IRrange`, n=`r max(filter(dat,Subpopulation=='IR2022'&Indicator=='DO')$NResp)`) of percentage of streams below Virginia's Water Quality Standard ( +/- confidence limits).
...
##### Table 4.4-2. pH results (`r IRrange`, n=`r max(filter(dat,Subpopulation=='IR2022'&Indicator=='pH')$NResp)`) of streams below or above Virginia's Water Quality Standard ( +/- confidence limits).
...
##### Table 4.4-3. Temperature results (`r IRrange`, n= `r max(filter(dat,Subpopulation=='IR2022'&Indicator=='pH')$NResp)`) above Virginia's Water Quality Standard ( +/- confidence limits).
...
acuteExceedances <- filter(metalsAssessment, Year >= 2015 & Year <= 2020 & Acute_Exceed==1) %>% 
  distinct(siteID) %>% 
  nrow()

# IR Window Chronic Exceedances
chronicExceedances <- filter(metalsAssessment, Year >= 2015 & Year <= 2020 & Chronic_Exceed==1)%>% 
  distinct(siteID) %>% 
  nrow()
```
...
##### Table 4.4-4. Dissolved metals results (`r IRrange`, n=`r max(filter(dat,Subpopulation=='IR2022'&Indicator=='ARSENIC')$NResp)`) above Virginia's Water Quality Standards ( +/- confidence limits). ppb^1^ = parts per billion.

```{r table4.4-4}


# copper mileage calculation, need to automate and not update ad hoc
copper <- filter(dat, Indicator == 'COPPER' & Subpopulation == 'IR2022') %>%
...
##### Table 4.4-6. VSCI/VCPMI (`r IRrange`, n=`r max(filter(dat,Subpopulation=='IR2022'&Indicator=='VSCIVCPMI')$NResp)`) Scores below Virginia's Assessment Thresholds ( +/- confidence limits).
...
##### Figure 4.4-7. Biological stream condition index based on VSCI/VCPMI Scores (`r IRrange`, n = `r  max(filter(dat,Subpopulation=='IR2022'&Indicator=='VSCIVCPMI')$NResp)`).
...
##### Figure 4.4-12. Estimate of habitat condition in Virginia streams and rivers and associated 95% confidence interval. Data presented is from `r IRrange` (n = `r  max(filter(dat,Subpopulation=='IR2022'&Indicator=='TotHab')$NResp)`).
...
##### Figure 4.4-14. Estimate of streambed sedimentation conditions in Virginia streams and rivers as defined by LRBS measures and 95% confidence interval. Data presented is from `r IRrange` (n = `r  max(filter(dat,Subpopulation=='IR2022'&Indicator=='LRBS')$NResp)`).
