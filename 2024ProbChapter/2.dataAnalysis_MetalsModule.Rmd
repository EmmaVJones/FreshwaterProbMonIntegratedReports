---
title: "IR2024 ProbMon Metals Analysis"
author: "Emma Jones"
...
source('C:/HardDriveBackup/R/GitHub/IR2024/3.automatedAssessment/automatedAssessmentFunctions.R')
source('C:/HardDriveBackup/R/GitHub/IR2024/3.automatedAssessment/updatedBacteriaCriteria.R')
...
stationsTable2022 <- readRDS('C:/HardDriveBackup/R/GitHub/IR2024/2.organizeMetadata/data/stationsTable2022.RDS')
stationsTable2020 <- readRDS('C:/HardDriveBackup/R/GitHub/IR2024//2.organizeMetadata/data/stationsTable2020.RDS')
```
...
lakeNutStandards <- read_csv('C:/HardDriveBackup/R/GitHub/IR2024/3.automatedAssessment/data/9VAC25-260-187lakeNutrientStandards.csv')
```
...
extraSites <- readRDS('C:/HardDriveBackup/R/GitHub/IR2024/3.automatedAssessment/data/extraSites.RDS')
```
...
## Citmon Data Level Summary

To expedite the citmon level summary part of the comments, we will organize all leveled data and summarize parameter counts by level here into a tibble so we can easily call this information at the time of creating the station comments.

```{r citmon level summary}
citmonLevelSummary <- dplyr::select(conventionals, STATION_ID = FDT_STA_ID, contains("LEVEL")) %>% 
  pivot_longer(-STATION_ID, names_to = 'parameter', values_to = 'level') %>% 
  filter(level %in% c('Level I', 'Level II', 'Level III')) %>% 
  group_by(STATION_ID, parameter, level) %>% 
  count() %>% 
  mutate(Summary = paste(level, n, sep = ': ')) %>% 
  arrange(desc(level)) %>% 
  mutate(parameter = str_remove(parameter, 'LEVEL_')) %>% 
  # fix some parameter names to be more human readable
  mutate(parameter = case_when(parameter == "70507" ~ 'PHOSPHORUS_TOTAL_ORTHOPHOSPHATE', 
                               parameter == "00630" ~ 'NITRITE+NITRATE_TOTAL',
                               parameter == "00615" ~ 'NITROGEN_NITRITE_TOTAL',
                               TRUE ~ as.character(parameter))) %>% 
  group_by(STATION_ID, parameter) %>% 
  summarise(`Parameter Summary` = paste0(parameter, " (", paste0(Summary, collapse = ", "), ")") ) %>% 
  distinct(`Parameter Summary`) %>% ungroup() %>% group_by(STATION_ID) %>% 
  summarise(`NonAgency Station Data Summary`= paste0(`Parameter Summary`, collapse = ", "))
```


Now bring in Probmon sites and limit the assessment run to just these sites.

```{r}
allData <- read_csv('processedData/Wadeable_ProbMon_2001-2022.csv') %>% 
  filter(Year %in% 2017:2022)


conventionals <- filter(conventionals, FDT_STA_ID %in% allData$StationID)
stationTable <- filter(stationTable, STATION_ID %in% allData$StationID)
```
...
filter(waterColumnMetalsAnalysis, `Exceedances in Rolled Window` >0) %>%
  dplyr::select(-associatedData) %>% 
  write.csv('processedData/metalsExceedances.csv')
```
...
# Automated Assessment Steps, full assessment
...
# if you want to know the ith number of a particular station, use this: i = which(stationTable$STATION_ID == '8-POR015.70', arr.ind=TRUE)
...
#i = 1
  ...
  if(any(nrow(stationData) > 0 | stationTable$STATION_ID[i] %in% extraSites$FDT_STA_ID)){
    ...
    results <- cbind(
      ...
      left_join(citmonLevelSummary, by = 'STATION_ID') %>% 
...
stationTableResults <- bind_rows(stationsTemplate, stationTableResults) %>%
  dplyr::select(STATION_ID:`Date Last Sampled`, `NonAgency Station Data Summary`, `2022 IR COMMENTS`:`7Q10 Flag`)

timeDiff = Sys.time()- startTime
```

Now save all this work and review for any exceedances

```{r}
# can use write_csv bc there are no date fields that could be corrupted by function
write_csv(stationTableResults, 'processedData/assessmentResults/stationTableResults06302023.csv',
...
#saveRDS(waterColumnMetalsAnalysis, 'processedData/assessmentResults/WCmetalsForApp.RDS')
```
