...
This document walks you through the steps of calculating design weights, running analyses, and writing the Integrated Report chapter for the Probabilistic Monitoring Program due to EPA every two years. This document overviews the 2024 IR ProbMon chapter.
...
Now time to find any exceedances of metals criteria for acute or chronic. In IR2024, the guidance changed on how we assess these parameters for Aquatic Life use. To avoid having to go  back to the original data, we can use the cleaned WCmetalsForAnalyisIR2024 pinned dataset and the same assessment logic to run through our probMon sites efficiently with the most up to date analysis method.

Since the assessment is programmed in 3.6.2 and needs to connect to the R server for pinned data, run this next chunk sourced code in 3.6.2!

See 2.dataAnalysis_MetalsModule.Rmd


```{r look for metals exceedances}
# how we ran the analysis previously
...
# filter(metalsAssessment, Year >= 2017 & Year <= 2022 & Acute_Exceed==1)
# 
# # IR Window Chronic Exceedances
# filter(metalsAssessment, Year >= 2017 & Year <= 2022 & Chronic_Exceed==1)
# 
# #write.csv(metalsAssessment, 'processedData/metalsAssessmentResults.csv')
```

After investgating each of the metals that exceeded in the riverine assessment app, here is a summary for the report to ingest.
...
designStatus <- read_csv('processedData/designStatusIR2024.csv') %>%
  mutate(
    ...
    BioPanel4 = ifelse(BioPanel == "Phase4", 1, NA),
    BioPanel5 = ifelse(BioPanel == "Phase5", 1, NA)) %>% # housekeeping: recode biophase and change 1's to NA's to indicate it wasnt sampled in that window, break up Panel and BioPanel windows to separate columns for weight adjustment purposes
...
rrisk <- left_join(surveyData,select(designStatus,siteID, Year,`Longitude-DD`,`Latitude-DD`)
  ...
  select(siteID:`Latitude-DD`,finalweight_all,TN,TP,TDS,LRBS,TotHab,VSCIVCPMI,MetalCCU, pH, DO)
...
#extras
rrisk$DOstatus <- cut(rrisk$DO,c(0,4,20),labels=c('Poor','Good'))  
#rrisk$DOstatus <- cut(rrisk$DO,c(0,4,8,20),labels=c('Poor','Fair','Good'))  
rrisk$pHstatus <- cut(rrisk$pH,c(0,6,9,14),labels=c('Poor','Good','Poor'))  
...
#extras
rrisk$DOstatus[ rrisk$DOstatus == "Fair" ] = NA
rrisk$pHstatus[ rrisk$pHstatus == "Fair" ] = NA
...
stres.vars<- c("TotHabstatus","TNstatus","TPstatus","TDSstatus","MetalCCUstatus","LRBSstatus", "DOstatus", 'pHstatus');
...
write.csv(relrisk.estimates, file = "processedData/relriskIR2024_withDOpH.csv", row.names = FALSE)
write.csv(rrisk, file = "processedData/rrisk_withDOpH.csv", row.names = FALSE)
...
allProb <- left_join(surveyData,designStatus,by=c('siteID','Year')) %>%
  select(siteID, Year, `strahler order`:`Latitude-DD`,Basin:IR2024) %>%
...
#st_write(allProb, 'processedData/GIS/allProb2024.shp')
...
allWshd <- st_read('C:/HardDriveBackup/GIS/ProbMonGIS/DelineatedWatersheds/Watersheds.gdb',
                   'AllWatersheds_through2022_burnEverythingElse') %>% 
...
# find boatable sites 
boatableSites <- filter(designStatus, status == 'NT' ) %>% 
  filter(str_detect(comment, 'Large') | str_detect(comment, 'Boat') | 
           str_detect(comment, 'large') | str_detect(comment, 'boat')) %>% 
...
st_write(boatableSites, "processedData/GIS/boatableSites2006_2022.shp")
st_write(boatableWshd, "processedData/GIS/boatableWshd2006_2022.shp")
