...
listOfResults <- function(popstatus.est, finalData, parameterName){
  list(
    ...
    estimateJU = subpopEstimate(finalData,parameterName, 'VAHUSB', 'JU','James River, Upper (Mountain)', specialWeight=FALSE),
    ...
    # change here each new cycle
    estimate2021 = subpopEstimate(finalData,parameterName, 'Year', '2021','Year 2021', specialWeight='finalweight_Year') ,
    estimate2022 = subpopEstimate(finalData,parameterName, 'Year', '2022','Year 2022', specialWeight='finalweight_Year') ,
    
    # change here each new cycle
    
    # By Phase
    estimatePhase1 = subpopEstimate(finalData,parameterName, 'Panel', 'Phase1','Phase One 2001-2011', specialWeight='finalweight_Panel1') ,
    estimatePhase2 = subpopEstimate(finalData,parameterName, 'Panel', 'Phase2','Phase Two 2012-2022', specialWeight='finalweight_Panel2') ,
    
    
    # change here each new cycle
    # Bay/ NonBay
    estimateBay = subpopEstimate(finalData,parameterName, 'BayShed', 'Bay','Bay Watersheds 2001-2022', specialWeight=FALSE),
    estimateNonBay = subpopEstimate(finalData,parameterName, 'BayShed', 'NonBay','Non-Bay Watersheds 2001-2022', specialWeight=FALSE),
    estimateBayPhase1 = subpopEstimate(finalData,parameterName, 'BayPanel', 'BayPhase1','Bay Watersheds 2001-2011', specialWeight='finalweight_Panel1'),
    estimateBayPhase2 = subpopEstimate(finalData,parameterName, 'BayPanel', 'BayPhase2','Bay Watersheds 2012-2022', specialWeight='finalweight_Panel2'),
    estimateNonBayPhase1 = subpopEstimate(finalData,parameterName, 'BayPanel', 'NonBayPhase1','Non-Bay Watersheds 2001-2011', specialWeight='finalweight_Panel1'),
    estimateNonBayPhase2 = subpopEstimate(finalData,parameterName, 'BayPanel', 'NonBayPhase2','Non-Bay Watersheds 2012-2022', specialWeight='finalweight_Panel2'),
    ...
    estimateBioPanelPhase5 = subpopEstimate(finalData,parameterName, 'BioPanel', 'Phase5','VSCI Scores 2021-2022', specialWeight='finalweight_BioPanel5'),
    ...
    # change here each new cycle
    estimateIR2024 = subpopEstimate(finalData,parameterName, 'IR2024', '2024','IR2024', specialWeight='finalweight_IR2024'),
...
allCDFresults <- function(designStatus, surveyData, parameter){
  ...
  trendWeightAdjustments <- mutate(initialWeightData,
    ...
    # 2024 IR
    group_by(siteIDoriginal, IR2024) %>%
    mutate(nYearsSampled_IR2024 = ifelse(is.na(IR2024),NA,n())) %>%
    ungroup() %>%
    ...
    # change here each new cycle, if adding new biopanel
    group_by(siteIDoriginal, BioPanel5) %>%
    mutate(nYearsSampled_BioPanel5 = ifelse(is.na(BioPanel5),NA,n())) %>%
    ungroup() %>% 
    ...
    mutate(designweight_all= designweightoriginal/nYearsSampled,
           # change here each new cycle
           designweight_IR2024= ifelse(is.na(nYearsSampled_IR2024),designweightoriginal,designweightoriginal/nYearsSampled_IR2024),
           ...
           # change here each new cycle, when new biopanel needed
           designweight_BioPanel5= ifelse(is.na(nYearsSampled_BioPanel5),designweightoriginal,designweightoriginal/nYearsSampled_BioPanel5))
  ...
  finalWeights <- select(trendWeightAdjustments,
                         siteID:IR2024, # change here each new cycle
                         siteIDoriginal,designweightoriginal,
                         designweight_all:designweight_BioPanel5, # change here each new cycle, when new biopanel needed
  ...
  # change here each new cycle
  finalWeights$finalweight_IR2024 <- adjwgt(rep(TRUE,length(finalWeights$designweight_IR2024)),finalWeights$designweight_IR2024,
                                            finalWeights$`strahler order`, sframe)
  ...
  finalWeights$finalweight_BioPanel4 <- adjwgt(rep(TRUE,length(finalWeights$designweight_BioPanel4)),finalWeights$designweight_BioPanel4,
                                               finalWeights$`strahler order`, sframe)
  # change here each new cycle, when new biopanel needed
  finalWeights$finalweight_BioPanel5 <- adjwgt(rep(TRUE,length(finalWeights$designweight_BioPanel5)),finalWeights$designweight_BioPanel5,
                                               finalWeights$`strahler order`, sframe)
